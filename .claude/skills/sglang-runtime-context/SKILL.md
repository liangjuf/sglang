...
- **Whole-object passes** (`f(server_args)` handing the instance along) keep the
  ...
  pins the remaining set — both spellings of the read, `server_args.field` and
  literal-name `getattr(server_args, "field", default)` — and fails on a new
  one, so the decision gets made when the read is written. Two shapes stay parameter-form on purpose: a helper the
