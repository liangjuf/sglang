...
def serving_attention_backend(forward_batch: ForwardBatch) -> str:
    ...
    Mirrors ``HybridAttnBackend._select_backend``: decode for decode/idle, the
    ``speculative_attention_mode`` half for verify and draft-extend-v2, prefill
    otherwise. The pair the runner stamped on its backend wins over the
    configured one, so a draft runner answers with its own backend.
    """
